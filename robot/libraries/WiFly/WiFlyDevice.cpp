...
void WiFlyDevice::SetNetwork(char *ipAddr, char *netMask){
  enterCommandMode();

  char stringSetIpAddr[100] = "set ip address ";
  char stringSetNetMask[100] = "set ip netmask ";

  strcat(stringSetIpAddr,ipAddr);
  strcat(stringSetNetMask,netMask);

  
  sendCommand(stringSetIpAddr);
  sendCommand(stringSetNetMask);
...
void WiFlyDevice::EnableUDPAndTCP(){
  enterCommandMode();
...
void WiFlyDevice::SetBroadcastInterval(byte interval){
  enterCommandMode();

  char stringInterval[3];

  itoa(interval, stringInterval,3);

  char cmd [100] = "set broadcast interval ";

  strcat(cmd, stringInterval);
...
boolean WiFlyDevice::joinAdHocNetwork(const char *ssid, char *ipAddr, char *netMask){
  DEBUG_LOG(1, "Entered WiFlyDevice::beginAdhoc()");
